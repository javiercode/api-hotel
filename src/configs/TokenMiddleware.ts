...
export const TokenMiddleware = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.originalUrl.toString().includes("/servicio2/") || req.originalUrl.toString().includes("api/v1/api") || req.originalUrl.toString().includes("/user/") || req.originalUrl.toString().includes("/login") || req.originalUrl.toString().includes("/usuario/create")) {
        next()
    } else {
        if (req.originalUrl.toString().includes("/user/getFoto") || req.originalUrl.toString().includes("/login") || aToken?.length == 2) {
            ...
            if (token && decodeToken.activo) {
                res = encodeToken(res, decodeToken.username, decodeToken.name, decodeToken.rol, decodeToken.rolId, decodeToken.aSucursal, decodeToken.sucursal, decodeToken.foto);
                next()
            } else {
                res.sendStatus(401);
            }
...
export const encodeToken = (res: Response, usuario: string, nombre: string, rol: string[], rolId: number[], aSucursal: number[], sucursal: number, foto: string) => {
...
export const getToken = (res: Response, usuario: string, nombre: string, rol: string[], rolId: number[], aSucursal: number[], sucursal: number, foto: string) => {
    const userForToken: JwtPayload = {
        username: usuario,
        name: nombre,
        rol: rol,
